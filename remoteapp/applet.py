...
class Applet(JMSBaseModel):
    ...
    def try_to_use_private_account(self, user, host, valid_accounts):
        # 获取用户的原始用户名并进行处理：加上 'js_' 前缀，并做截取处理
        username = 'js_' + user.username  # 改造部分
        # 如果用户名超过17个字符，则截取前10位和后7位，确保总长度不超过20位
        if len(username) > 17:  # 改造部分
            username = username[:10] + username[-7:]  # 改造部分
        ...
        # 查找经过截取处理后的私有账户
        private_account = valid_accounts.filter(username=username).first()  # 改造部分 此处修改获取username的方法

        if not private_account:
            logger.debug('Private account not found for username: {}'.format(username))
            return None

        # 如果都支持并发或者私有账号没有被占用，则使用私有账号
        account = None
        if all_can_concurrent:
            logger.debug('All can concurrent, use private account')
            account = private_account
        elif not host_can_concurrent:
            # 如果主机不支持并发，查询私有账号是否已被占用
            ...
            # 如果主机支持并发但应用不支持并发，检查私有账号是否已被当前应用使用
            ...
            logger.debug("Private account is using by {}: {}".format(self.name, private_is_using_by_this_app))
